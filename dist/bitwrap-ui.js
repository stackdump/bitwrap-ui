...
},{"./src/stilts-stomp":10,"./src/svg-machine.coffee":11,"bitwrap-npm":2,"jquery":5,"snapsvg":6}],2:[function(require,module,exports){
var Dsl, Machine;

Machine = require('./bitwrap/machine');

Dsl = require('./bitwrap/dsl/machine');

this.schemata = {};

exports.set_schema_path = function(dirname) {
  return this.schema_path = dirname;
};

exports.open = function(schema) {
  var base;
  return (base = this.schemata)[schema] || (base[schema] = new Machine(Dsl, schema, require(this.schema_path + "/" + schema)));
};

},{"./bitwrap/dsl/machine":3,"./bitwrap/machine":4}],3:[function(require,module,exports){
var obj_at_index, transact_to_vector,
  indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

exports.storage = function(schema) {
  return {};
};
...
exports.places = function(place_data, roles) {
  var label, p, place, places, r;
  places = {};
  for (label in place_data) {
    place = place_data[label];
    p = places[label] = [];
    p[0] = indexOf.call(roles, label) >= 0 ? 0 : 1;
    p[1] = place['token'];
    p[2] = (function() {
      var j, len, results;
      results = [];
...
transact_to_vector = function(tx, role, places, roles) {
  var from_address, i, k, key, op, p, p0, p1, params, ref, ref1, ref2, role_id, send_from, send_to, to_address, txn, val, valid, vector;
  vector = {};
  for (key in places) {
    p = places[key];
    if (key === role) {
  ...
  role_id = roles.indexOf(role);
  for (i in tx) {
    ref = tx[i], op = ref[0], txn = ref[1];
    valid = false;
    params = (function() {
      var results;
      results = [];
    ...
    (ref1 = params[0], send_from = ref1[0], from_address = ref1[1]), (ref2 = params[params.length - 1], send_to = ref2[0], to_address = ref2[1]);
    if (op === 'transfer' || op === 'debit' || op === 'destroy') {
      places[send_from][2][role_id] = 'sender';
      vector[send_from] = -1;
      valid = true;
    }
    if (op === 'transfer' || op === 'credit' || op === 'create') {
      places[send_to][2][role_id] = 'target';
      vector[send_to] = 1;
      valid = true;
    }
    if (!valid) {
      throw new Error('__UNKNOWN_OPERATION__');
    }
  }
  return (function() {
    var results;
...
exports.transitions = function(data, places, roles) {
  var label, transitions, tx;
  transitions = {};
  for (label in data) {
    tx = data[label];
    transitions[label] = transact_to_vector(tx['transact'], tx['role'], places, roles);
  }
  return transitions;
};
...
Machine = (function() {
  function Machine(dsl, schema, machine) {
    this.transform = bind(this.transform, this);
    this.execute = bind(this.execute, this);
    this.fetch = bind(this.fetch, this);
    this.new_request = bind(this.new_request, this);
    this.storage = dsl.storage(schema);
    this.roles = dsl.roles(machine['roles']);
    this.places = dsl.places(machine['places'], machine['roles']);
    this.transitions = dsl.transitions(machine['transitions'], this.places, this.roles);
  }

  Machine.prototype.inital_state = function() {
    var label, p, ref, results;
    ref = this.places;
    results = [];
    for (label in ref) {
      p = ref[label];
  ...
  Machine.prototype.null_action = function() {
    var i, p, ref, results;
    ref = this.places;
    results = [];
    for (i in ref) {
  ...
  Machine.prototype.control_vector = function(role_id) {
    var empty_state;
    empty_state = this.null_action();
    empty_state[role_id] = 1;
    return empty_state;
  };

  Machine.prototype.new_request = function(msg) {
    var action, errors, id, label, role_id;
    errors = [];
    label = msg['signal']['action'];
    action = this.transitions[label];
    action || (action = false);
    if (label !== 'OP_RETURN') {
      if (!action) {
        errors.push([
          '__UNKNOWN_ACTION__', {
            'description': "Unknown action: " + label
          }
        ]);
      }
    } else {
    ...
    return {
      ...
      'message': msg,
      'cache': {
        'control': this.control_vector(role_id)
      }
    };
  };

  Machine.prototype.fetch = function(key, default_val) {
    var v;
    v = this.storage[key];
    if (void 0 === v) {
      return default_val;
  ...
  Machine.prototype.execute = function(request, role_offset) {
    var address, address_key, base, color, delta, inital_value, label, offset, ref, ref1, role_map;
    if (role_offset == null) {
      role_offset = 0;
    }
    offset = 0;
    ref = this.places;
    for (label in ref) {
      ref1 = ref[label], inital_value = ref1[0], color = ref1[1], role_map = ref1[2];
      address_key = role_map[role_offset];
      if (address_key === 'control') {
        address = address_key;
      } else {
        address = request['message']['addresses'][address_key];
      }
      (base = request['context'])[address_key] || (base[address_key] = this.null_action());
      if (void 0 === request['cache'][address]) {
        request['cache'][address] = this.fetch(address, this.inital_state());
      }
      delta = request['context']['action'][offset];
      if (void 0 === request['cache'][address]) {
        request['errors'].push(['__CACHE_MISS__', address]);
      } else {
        request['context'][address_key][offset] = delta;
        request['cache'][address][offset] += delta;
        if (request['cache'][address][offset] < 0) {
          request['errors'].push(['__INSUFFICENT_BALANCE__', address]);
        }
      }
      offset += 1;
    }
  ...
  Machine.prototype.transform = function(msg) {
    return this.execute(this.new_request(msg));
  };
...
Elements = (function() {
  function Elements(bx) {
    var Circle, Rect, Text, Token, Transition, _r, _scale, _w, _x, _y, graph_x, graph_y, label, p, ref, ref1, ref2, s, t, tx, tx_l, tx_r, tx_w;
    Text = function(label, x, y, body, attribs) {
      return bx.o(label, bx.s.text(x, y, body)).attr(attribs);
    };
    Rect = function(label, x, y, w, h, rx, ry, attribs) {
      return bx.o(label, bx.s.rect(x, y, w, h, rx, ry)).attr(attribs);
    };
    Circle = function(label, cx, cy, r, attribs) {
      return bx.o(label, bx.s.circle(cx, cy, r)).attr(attribs);
    };
    Token = Circle;
    Transition = Rect;
    ...
    _w = 2;
    _x = function(v) {
      return graph_x + v * _scale;
    };
    _y = function(v) {
      return graph_y + v * _scale;
    };
    ref = bx.machine.tokens;
    for (label in ref) {
      t = ref[label];
...
Graph = (function() {
  function Graph(machine) {
    ...
    this.edges = [];
    this.edge_names = [];
    this.tokens = machine['tokens'];
  }

  Graph.prototype.place_to_node = function(p) {};

  Graph.prototype.transition_to_node = function(t) {};

  Graph.prototype.transition_to_edges = function(t) {};
...
SvgMachine = (function() {
  function SvgMachine(svg, schema, data, client) {
    ...
    this.machine = data;
    this.stomp = client;
    this.graph = new Graph(this.machine);
  }

  SvgMachine.prototype.o = function(key, val) {
    if (val === void 0) {
  ...
  SvgMachine.prototype.render = function() {
...
},{"./elements.coffee":8,"./graph.coffee":9}]},{},[1]);
